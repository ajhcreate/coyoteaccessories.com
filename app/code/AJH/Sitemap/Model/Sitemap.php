...
class Sitemap extends CoreSitemap {
    ...
    Context $context, Registry $registry, Escaper $escaper, Data $sitemapData,
            Filesystem $filesystem, CatalogCategoryFactory $categoryFactory,
            CatalogProductFactory $productFactory, CmsPageFactory $cmsFactory,
            PageFactory $corePageFactory, ProductFactory $coreProductFactory,
            CategoryFactory $coreCategoryFactory, Item $stockItem,
            DateTime $modelDate, StoreManagerInterface $storeManager,
            RequestInterface $request, StdlibDateTime $dateTime,
        ...
        $this->logger->info('AJH Sitemap:', 'Sitemap');
        
        parent::__construct(
                $context, $registry, $escaper, $sitemapData, $filesystem, $categoryFactory, $productFactory, $cmsFactory, $modelDate, $storeManager, $request, $dateTime, $resource, $resourceCollection, $data
        );
    }
    ...
    public function generateXml() {
        ...
        foreach ($this->_sitemapItems as $item) {
            ...
            $this->logger->info("generateXml count - " . count($item->getCollection()));

            foreach ($item->getCollection() as $key => $itemChild) {
                $this->logger->info("generateXml - " . $itemChild->getUrl());
                $this->logger->info("generateXml URL - " . $urlType);
                $this->logger->info("generateXml getUpdatedAt - " . $itemChild->getUpdatedAt());
                $xml = $this->getSitemapRow(
                        $itemChild->getUrl(), $urlType, $itemChild->getUpdatedAt(), $changefreq, $priority, $itemChild->getImages()
                );
                ...
                $this->logger->info("generateXml getUpdatedAt - " . $key);
            }
        }

        $xml2 = $this->getSitemapRow(
                'custom-url', '1', '2019-10-26 02:20:20', 'daily', '0.5', null
        );
        if ($this->_isSplitRequired($xml2) && $this->_sitemapIncrement > 0) {
            $this->_finalizeSitemap();
        }
        if (!$this->_fileSize) {
            $this->_createSitemap();
        }
        $this->_writeSitemapRow($xml2);

        $xml3 = $this->getSitemapRow(
                'fitment/2021-acura-ilx-base', '1', date("Y-m-d H:m:s"), 'daily', '0.5', null
        );
        if ($this->_isSplitRequired($xml3) && $this->_sitemapIncrement > 0) {
            $this->_finalizeSitemap();
        }
        if (!$this->_fileSize) {
            $this->_createSitemap();
        }
        $this->_writeSitemapRow($xml3);


        // Increase counters
//                $this->_lineCount++;
//                $this->_fileSize += strlen($xml2);
//        $xml = $this->getSitemapRow(
//                'http://google.com/', '', date(), 'daily', '0.5', null
//        );
//        if ($this->_isSplitRequired($xml) && $this->_sitemapIncrement > 0) {
//            $this->_finalizeSitemap();
//        }
...
//        $this->_fileSize += strlen($xml);
    ...
    $url, $urlType, $lastmod = null, $changefreq = null, $priority = null,
            $images = null
    ) {
    ...
    public function _getPageCollection($storeId) {
        ...
        foreach ($this->_cmsFactory->create()->getCollection($storeId) as $item) {
            if ($this->_corePageFactory->create()->load($item->getId())->getData('mp_exclude_sitemap') == 1 || $this->optimizeHomepage($storeId, $item)
            ) {
                continue;
            }
            $collection[] = $item;
    ...
    public function optimizeHomepage($storeId, $page) {
        return $this->helperConfig->isEnableHomepageOptimization($storeId) == 1 && $this->helperConfig->getConfigValue(self::HOMEPAGE_PATH, $storeId) == $page->getUrl();
    }
