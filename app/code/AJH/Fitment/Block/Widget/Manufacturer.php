...
class Manufacturer extends Template implements BlockInterface {
    ...
    public function __construct(Context $context, Fitment $fitment) {
        ...
        $this->fitment_year = $this->fitment->_year; //$this->getRequest()->getParam('year');
        $this->fitment_make = $this->fitment->_make; //$this->getRequest()->getParam('make');

        if (!$this->fitment_make) {
            $this->fitment_make = $this->fitment->_make; //$this->getRequest()->getParam('makeid');
        }

        $this->fitment_model = $this->fitment->_model; //$this->getRequest()->getParam('model');

        if (!$this->fitment_model) {
            $this->fitment_model = $this->fitment->_model; //$this->getRequest()->getParam('modelid');
        }

        $this->fitment_submodel = $this->fitment->_submodel; //$this->getRequest()->getParam('submodel');

        if (!$this->fitment_submodel) {
            $this->fitment_submodel = $this->fitment->_submodel;//$this->getRequest()->getParam('submodelid');
        }
    }
